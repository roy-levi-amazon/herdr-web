...
fn main() -> io::Result<()> {
    ...
    if args.get(1).map(|s| s.as_str()) == Some("web-bridge") {
        std::process::exit(web_bridge::run_command(&args[2..])?);
    }
    ...
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("       herdr web-bridge [--host 127.0.0.1] [--port 8787]");
        ...
        for (command, description) in [
            ...
            ("herdr web-bridge", "Run the local web bridge"),
            (
